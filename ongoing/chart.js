...
d3.csv("boston_311_2023_by_reason.csv").then(data => {
    ...
    // Create Scales
    // =========================
    // Create the x-axis scale (categorical)
    const x = d3.scaleBand()
        .domain(data.map(d => d.reason)) // Use the sorted and sliced data
        .range([0, width])
        .padding(0.2);

    // Create the y-axis scale (numerical)
    ...
    // Add Axes
    // =========================
    // Add the x-axis (reason categories)
    svg.append("g")
    ...
    svg.selectAll(".bar")
      ...
      .enter().append("rect")
        .attr("class", "bar")
        .attr("x", d => x(d.reason)) // Align bars based on the categorical axis
        .attr("y", d => y(d.Count)) // Start bars at the appropriate y position
        .attr("width", x.bandwidth()) // Set width based on band size
        .attr("height", d => height - y(d.Count)) // Height corresponds to the Count
    ...
    function updateChart() {
        ...
        x.domain(filteredData.map(d => d.reason));
    ...
    d3.select('#show-more')
        .on('click', () => {
            ...
            d3.csv("boston_311_2023_by_reason.csv").then(fullData => {
                ...
                // Update y scale domain if necessary
